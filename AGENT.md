# AGENT.md — как писать новые сказки про мышонка Мыша
...
Память мышонка лежит в двух файлах. Прочитай оба целиком перед каждой новой сказкой:
...
- **id = английское название** существа или вещи: `grasshopper`, `spiderweb`, `spider`. Строчными, без пробелов (если два слова — через дефис: `water-strider`).
- id **стабилен для всех языков** и попадает в URL (`/creatures/grasshopper`). При переводе сказок на другие языки id не меняется — меняется только человекочитаемый текст.
...
### Имена существ и вещей

Существ и вещи называем в **базовой форме, без уменьшительно-ласкательных**: Паук (не Паучок), Мышь (не Мышка), Осёл (не Ослик), Жук (не Жучок), Лягушка (не Лягушонок), Паутина (не паутинка). Это касается и текста сказки, и полей `name` / `leadsToCharacter` в памяти.
...
4. Перечисли новых героев и вещи в сказке по их **id**.
5. Если познакомили кого-то, кто раньше был в `leadsToCharacter`, — он теперь полноценный друг; поле-предвестие можно убрать.
...
- **Проверка:** перед коммитом запусти `npm run build`. Память валидируется по схеме — пропущенное или неверное поле остановит сборку с точечной ошибкой (например, `Память (друзья) → «water-strider»: metInStory Required`). Это нормально: исправь поле и собери снова.
...
- [ ] Упомянуты совы / время суток согласовано.
...
- [ ] Память обновлена (YAML + файл сказки), id на английском.
